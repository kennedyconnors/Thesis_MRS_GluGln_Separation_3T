...
TE1_indices = [6, 9, 13, 17];  
TE2_indices = [6, 9, 13, 17];
...
Specs(:,1) = zeros(size(SpecAll,1),1);          % Glutamate (clean)
Specs(:,2) = zeros(size(SpecAll,1),1);          % Glutamine (clean)
Specs_with_noise(:,1) = zeros(size(SpecAll,1),1);  % Glutamate (noisy)
Specs_with_noise(:,2) = zeros(size(SpecAll,1),1);  % Glutamine (noisy)

% Sum spectra over selected TE values
for i = 1:4
    Specs(:,1) = Specs(:,1) + SpecAll(:,TE1_indices(i),TE2_indices(i), 1);
    Specs(:,2) = Specs(:,2) + SpecAll(:,TE1_indices(i),TE2_indices(i), 2);
...
Specs(:,1) = Specs(:,1) / 4;
Specs(:,2) = Specs(:,2) / 4;
Specs_with_noise(:,1) = Specs_with_noise(:,1) / 4;
Specs_with_noise(:,2) = Specs_with_noise(:,2) / 4;
...
for comp =1: SimPars.ncompounds
   ...
   D(:,2*comp-1) = reshape(Specs(:, comp),1,[]) .* exp(SimPars.R2 * t) .* exp(-SimPars.R2_2 * avg_TE * 1e-3);
D(:,2*comp)   = SimPars.amp * reshape(Specs(:, comp),1,[]) .* exp(SimPars.R2 * t) .* t .* exp(-SimPars.R2_2 * avg_TE * 1e-3);
end
